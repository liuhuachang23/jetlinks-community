package org.jetlinks.community.network.coap.codec;
...
public class CoapRuleDataCodec implements RuleDataCodec<CoapMessage> {
        ...
        if (!CollectionUtils.isEmpty(options)) {
            return options
                ...
                .map(kv -> {
                    ...
                    if (OptionNumberRegistry.getFormatByNr(num) == OptionNumberRegistry.OptionFormat.INTEGER) {
    ...
    public static Map<String, Object> toOptions(Collection<Option> options) {
        ...
        for (Option option : options) {
            Object val;
            OptionNumberRegistry.OptionFormat formats = OptionNumberRegistry.getFormatByNr(option.getNumber());
            if (formats == OptionNumberRegistry.OptionFormat.INTEGER) {
                val = option.getIntegerValue();
            } else {
                val = option.getStringValue();
            }
            if (formats != OptionNumberRegistry.OptionFormat.UNKNOWN) {
