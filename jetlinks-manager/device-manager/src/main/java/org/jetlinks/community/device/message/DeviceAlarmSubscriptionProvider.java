...
@ConfigurationProperties(prefix = "jetlinks.messaging.device-alarm-subscriber")
public class DeviceAlarmSubscriptionProvider implements SubscriptionProvider {
    ...
    @Generated
    public String id() {
        return "device-alarm-subscriber";
    }
    ...
    public Flux<Message> subscribe(SubscribeRequest request) {
        return eventBus
            .subscribe(
                Subscription.of(
                    "DeviceAlarmSubscriptionProvider:" + request.getAuthentication().getUser().getId(),
